...
async def send_all_first_reply(
    func: str, arg: Any, peers: List[WSDogeChiaConnection], timeout=15
) -> Optional[Tuple[Any, WSDogeChiaConnection]]:
    """performs an API request to peers and returns the result of the first response and the peer that sent it."""

    async def do_func(peer_x: WSDogeChiaConnection, func_x: str, arg_x: Any):
...
async def send_to_random(func: str, arg: Any, peers: List[WSDogeChiaConnection]) -> Optional[Tuple[Any, WSDogeChiaConnection]]:
    """performs an API request to peers and returns the result of the first response and the peer that sent it."""
