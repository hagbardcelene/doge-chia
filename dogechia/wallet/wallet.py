...
from dogechia.wallet.sign_coin_solutions import sign_coin_solutions
...
class Wallet:
    ...
    async def create_spend_bundle_relative_dogechia(self, dogechia_amount: int, exclude: List[Coin]) -> SpendBundle:
        list_of_solutions = []
        utxos = None

        # If we're losing value then get coins with at least that much value
        # If we're gaining value then our amount doesn't matter
        if dogechia_amount < 0:
            utxos = await self.select_coins(abs(dogechia_amount), exclude)
        else:
        ...
        dogechia_amount = spend_value + dogechia_amount
        ...
        for coin in utxos:
            ...
            if output_created is None:
                ...
                primaries = [{"puzzlehash": newpuzhash, "amount": dogechia_amount}]
