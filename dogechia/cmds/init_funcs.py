...
from dogechia.ssl.create_ssl import generate_ca_signed_cert, get_dogechia_ca_crt_key, make_ca_cert
...
from dogechia.wallet.derive_keys import master_sk_to_pool_sk, master_sk_to_wallet_sk
...
def check_keys(new_root: Path) -> None:
    ...
    for i in range(number_of_ph_to_search):
        ...
        for sk, _ in all_sks:
            ...
            if all_targets[-1] == config["farmer"].get("xdg_target_address"):
                stop_searching_for_farmer = True
            if all_targets[-1] == config["pool"].get("xdg_target_address"):
                stop_searching_for_pool = True

    # Set the destinations
    if "xdg_target_address" not in config["farmer"]:
        print(f"Setting the xdg destination address for coinbase fees reward to {all_targets[0]}")
    ...
    if "xdg_target_address" not in config["pool"]:
        print(f"Setting the xdg destination address for coinbase reward to {all_targets[0]}")
...
def create_all_ssl(root: Path):
    ...
    dogechia_ca_crt_path.write_bytes(dogechia_ca_crt)
    dogechia_ca_key_path.write_bytes(dogechia_ca_key)
    ...
    dogechia_ca_crt, dogechia_ca_key = get_dogechia_ca_crt_key()
    generate_ssl_for_nodes(ssl_dir, dogechia_ca_crt, dogechia_ca_key, False, overwrite=False)
...
def init(create_certs: Optional[Path], root_path: Path):
    if create_certs is not None:
        if root_path.exists():
            ...
            if dogechia_init(root_path) == 0 and root_path.exists():
                return init(create_certs, root_path)

            print(f"** {root_path} was not created. Exiting **")
            return -1
    else:
        return dogechia_init(root_path)
...
def dogechia_minor_release_number():
    res = int(dogechia_version_number()[2])
...
def dogechia_init(root_path: Path):
    if os.environ.get("DOGECHIA_ROOT", None) is not None:
        print(
            f"warning, your DOGECHIA_ROOT is set to {os.environ['DOGECHIA_ROOT']}. "
            f"Please unset the environment variable and run dogechia init again\n"
