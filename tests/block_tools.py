...
from dogechia.cmds.init_funcs import create_all_ssl, create_default_dogechia_config
...
from dogechia.full_node.generator import setup_generator_args
from dogechia.full_node.mempool_check_conditions import GENERATOR_MOD
from dogechia.plotting.create_plots import create_plots
...
from dogechia.consensus.coinbase import create_puzzlehash_for_pk, create_farmer_coin, create_pool_coin
...
from dogechia.plotting.plot_tools import PlotInfo, load_plots, parse_plot_info
...
from dogechia.util.condition_tools import ConditionOpcode, conditions_by_opcode
...
class BlockTools:
    ...
    def __init__(
        ...
        if len(self.pool_pubkeys) == 0 or len(farmer_pubkeys) == 0:
            raise RuntimeError("Keys not generated. Run `dogechia generate keys`")
    ...
    def init_plots(self, root_path: Path):
        ...
        try:
            ...
            args.pool_contract_address = encode_puzzle_hash(self.pool_ph, "xdg")
...
def get_plot_dir() -> Path:
    cache_path = Path(os.path.expanduser(os.getenv("DOGECHIA_ROOT", "~/.dogechia/"))) / "test-plots"
    mkdir(cache_path)
    return cache_path
