...
from dogechia.protocols.full_node_protocol import RespondBlock
...
from dogechia.simulator.simulator_protocol import FarmNewBlockProtocol, ReorgProtocol
...
class TestPoolWalletRpc:
    ...
    def create_pool_plot(self, p2_singleton_puzzle_hash: bytes32, shuil=None) -> bytes32:
        ...
        args.pool_contract_address = encode_puzzle_hash(p2_singleton_puzzle_hash, "txdg")
    ...
    async def test_absorb_pooling(self, one_wallet_node_and_rpc):
        ...
        # Balance stars at 6 XDG and 5 more blocks are farmed, total 22 XDG
    ...
    async def test_leave_pool(self, setup):
        ...
        try:
            ...
            async def have_dogechia():
                await self.farm_blocks(full_node_api, our_ph, 1)
                return (await wallets[0].get_confirmed_balance()) > 0

            await time_out_assert(timeout=WAIT_SECS, function=have_dogechia)
