...
This repo includes the code for the DogeChia full node, farmer, and timelord (in dogechia folder), which are all written in python.
It also includes a verifiable delay function implementation that it imports from the [chiavdf repo](https://github.com/DogeChia/chiavdf) (in c/c++), and a proof of space implementation that it imports from the [chiapos repo](https://github.com/DogeChia/chiapos). BLS signatures are imported from the [bls-signatures repo](https://github.com/DogeChia/bls-signatures) as blspy. There is an additional dependency on the [chiabip158 repo](https://github.com/DogeChia/chiabip158). For major platforms, binary and source wheels are shipped to PyPI from each dependent repo. Then dogechia-blockchain can pip install those from PyPI or they can be prepackaged as is done for the Windows installer. On unsupported platforms, pip will fall back to the source distributions, to be compiled locally.
...
Members of the DogeChia organization may create feature branches from the `main` branch.

In the event an emergency fix is required for the release version of DogeChia, members of the DogeChia organization will create a feature branch from the current release branch `1.0.0`.
...
4. DogeChia Network's nodes on the public testnet are running the latest version `x.y.z`.
...
. ./activate
pip install ".[dev]"
black dogechia tests && mypy dogechia tests && flake8 dogechia tests
...
7. Preferences > Settings > mypy > Targets: set to `./dogechia` and `./tests`
...
6. Install these linters https://github.com/DogeChia/dogechia-blockchain/tree/main/.github/linters

## Testnets and review environments

With the launch of `1.0.0` we will begin running an official `testnet`.  
Prior to the release of `1.1.0` there will be two running test nets. `testnet` and `transaction-beta-testnet`. The `transaction-beta-testnet` testnet will be a beta of the pending 1.1 release, which will enable transactions on the dogechia blockchain.
...
DogeChia organization members have been granted CI access to deploy `beta testnets`.
If you are not a DogeChia organization member, you can enquire about deploying a `beta testnet` in the public dev Discord channel.

## Submit changes

To propose changes, please make a pull request to the `main` branch. See Branching Strategy above.

To propose changes for the production releases of DogeChia, please make a pull request to the latest release branch.
