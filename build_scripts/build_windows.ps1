...
Write-Output "   ---"
Write-Output "Get DOGECHIA_INSTALLER_VERSION"
# The environment variable DOGECHIA_INSTALLER_VERSION needs to be defined
$env:DOGECHIA_INSTALLER_VERSION = python .\build_scripts\installer-version.py -win
...
Write-Output "   ---"
Write-Output "Build dogechia-blockchain wheels"
Write-Output "   ---"
pip wheel --use-pep517 --extra-index-url https://pypi.chia.net/simple/ -f . --wheel-dir=.\build_scripts\win_build .

Write-Output "   ---"
Write-Output "Install dogechia-blockchain wheels into venv with pip"
Write-Output "   ---"
...
Write-Output "pip install dogechia-blockchain"
pip install --no-index --find-links=.\win_build\ dogechia-blockchain
...
Write-Output "   ---"
Copy-Item "dist\daemon" -Destination "..\dogechia-blockchain-gui\" -Recurse
...
electron-packager . DogeChia --asar.unpack="**\daemon\**" --overwrite --icon=.\src\assets\img\dogechia.ico --app-version=$packageVersion
Write-Output "   ---"
...
If ($env:HAS_SECRET) {
   ...
   Write-Output "   ---"
   signtool.exe timestamp /v /t http://timestamp.comodoca.com/ .\release-builds\windows-installer\DogeChiaSetup-$packageVersion.exe
   signtool.exe verify /v /pa .\release-builds\windows-installer\DogeChiaSetup-$packageVersion.exe
   }   Else    {
